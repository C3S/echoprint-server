...
class Response(object):
    # Response codes
    NOT_ENOUGH_CODE, CANNOT_DECODE, SINGLE_BAD_MATCH, SINGLE_GOOD_MATCH, \
        NO_RESULTS, MULTIPLE_GOOD_MATCH_HISTOGRAM_INCREASED, \
        MULTIPLE_GOOD_MATCH_HISTOGRAM_DECREASED, \
        MULTIPLE_BAD_HISTOGRAM_MATCH, MULTIPLE_GOOD_MATCH = list(range(9))
    ...
    def message(self):
        ...
        if self.code == self.SINGLE_BAD_MATCH or self.code == self.NO_RESULTS \
...
def inflate_code_string(s):
    ...
    # 5 hex bytes for hash, 5 hex bytes for time (40 bits)
    n = int(len(s) / 10.0)

    def pairs(k, n=2):
        """Non-overlapping [1,2,3,4] -> [(1,2), (3,4)]"""
        # return zip(*[[v for i,v in enumerate(l) if i % n == j]
        #              for j in range(n)])
        end = n
        res = []
        while end <= len(k):
            start = end - n
            res.append(tuple(k[start:end]))
            end += n
        return res

    # Parse out n groups of 5 timestamps in hex; then n groups of 8 hash codes
    # in hex.
    end_timestamps = n*5
...
def decode_code_string(compressed_code_string):
    if type(compressed_code_string) is str:
    ...
    try:
        ...
        actual_code = zlib.decompress(
            base64.urlsafe_b64decode(compressed_code_string)).decode()
    except (zlib.error, TypeError):
        logger.warn("Could not decode base64 zlib string %s"
                    % (compressed_code_string))
        import traceback
...
def cut_code_string_length(code_string):
    ...
    # If we use the codegen on a file with start/stop times, the first
    # timestamp is ~= the start time given. There might be a (slightly) earlier
    # timestamp in another band, but this is good enough
...
def best_match_for_query(code_string, elbow=10, local=False):
    ...
    if re.match(b'[A-Za-z\/\+\_\-]', code_string) is not None:  # noqa
    ...
    if code_len < elbow:
        logger.warn("Query code length (%d) is less than elbow (%d)"
                    % (code_len, elbow))
    ...
    if len(response.results) == 0:
        return Response(Response.NO_RESULTS, qtime=response.header["QTime"],
                        tic=tic)

    # If we just had one result, make sure that it is close enough. We rarely
    # if ever have a single match so this is not helpful (and probably doesn't
    # work well.)
    ...
    if len(response.results) == 1:
        ...
        if code_len - top_match_score < elbow:
            ...
            return Response(Response.SINGLE_BAD_MATCH,
                            qtime=response.header["QTime"], tic=tic)

    # If the scores are really low (less than 5% of the query length) then say
    # no results
    if top_match_score < code_len * 0.05:
        return Response(Response.MULTIPLE_BAD_HISTOGRAM_MATCH,
                        qtime=response.header["QTime"], tic=tic)

    # Not a strong match, so we look up the codes in the keystore and compute
    # actual matches...
    ...
    # For each result compute the "actual score" (based on the histogram
    # matching)
    for (i, r) in enumerate(response.results):
        ...
        if track_code is None:
            ...
            continue
        if type(track_code) is bytes:
            track_code = track_code.decode()
        actual_scores[track_id] = actual_matches(code_string, track_code,
                                                 elbow=elbow)

    # logger.debug("Actual score for %s is %d (code_len %d), original was %d"
    #              % (r["track_id"], actual_scores[r["track_id"]], code_len,
    #                 top_match_score))
    # Sort the actual scores
    sorted_actual_scores = sorted(iter(actual_scores.items()), key=key,
                                  reverse=True)

    # Because we split songs up into multiple parts, sometimes the results
    # will have the same track in the first few results. Remove these
    # duplicates so that the falloff is (potentially) higher.
    ...
    if len(sorted_actual_scores) == 1:
        ...
        if top_score < code_len * 0.1:
            logger.info("only result less than 10%% of the query string "
                        "(%d < %d *0.1 (%d)) SINGLE_BAD_MATCH", top_score,
                        code_len, code_len*0.1)
            return Response(Response.SINGLE_BAD_MATCH,
                            qtime=response.header["QTime"], tic=tic)
        else:
            if top_score > (original_scores[top_track_id] / 2):
                logger.info("top_score > original_scores[%s]/2 (%d > %d) "
                            "GOOD_MATCH_DECREASED", top_track_id, top_score,
                            original_scores[top_track_id]/2)
                trid = top_track_id.split("-")[0]
                meta = metadata_for_track_id(trid, local=local)
                return Response(
                    Response.MULTIPLE_GOOD_MATCH_HISTOGRAM_DECREASED,
                    TRID=trid, score=top_score, qtime=response.header["QTime"],
                    tic=tic, metadata=meta)
            else:
                logger.info("top_score NOT > original_scores[%s]/2 (%d <= %d) "
                            "BAD_HISTOGRAM_MATCH", top_track_id, top_score,
                            original_scores[top_track_id]/2)
                return Response(Response.MULTIPLE_BAD_HISTOGRAM_MATCH,
                                qtime=response.header["QTime"], tic=tic)

    # Get the top one
    (actual_score_top_track_id, actual_score_top_score) = \
        sorted_actual_scores[0]
    # Get the 2nd top one (we know there is always at least 2 matches)
    (actual_score_2nd_track_id, actual_score_2nd_score) = \
        sorted_actual_scores[1]
    ...
    if actual_score_top_score < code_len * 0.05:
        ...
        # If the actual score went down it still could be close enough, so
        # check for that
        if actual_score_top_score > (
                original_scores[actual_score_top_track_id] / 4):
            if (actual_score_top_score - actual_score_2nd_score) >= (
                    actual_score_top_score / 3):
...
"""
    fp can query the live production flat or the alt flat, or it can query and
    ingest in memory.
    the following few functions are to support local query and ingest that ape
    the response of the live server
    This is useful for small collections and testing, deduplicating, etc,
    without having to boot a server.
...
class FakeSolrResponse(object):
    def __init__(self, results):
        ...
        self.results = []
        for r in results:
            # If the result list has more than 2 elements we've asked for data
            # as well
            if len(r) > 2:
...
def local_query_fp(code_string, rows=10, get_data=False):
    ...
    if not get_data:
        ...
        return FakeSolrResponse(sorted(iter(top_matches.items()), key=key,
                                       reverse=True)[0:rows])
    else:
...
def split_codes(fp):
    ...
    at most 60 seconds of codes, and codes overlap every 30 seconds. Given
    a track id, return track ids of the form trid-0, trid-1, trid-2, etc.
    """
...
def ingest(fingerprint_list, do_commit=True, local=False, split=True):
    """ Ingest some fingerprints into the fingerprint database.
        ...
        or a list of the same. All parameters except length must be strings.
        Length is an integer.
        artist, release and track are not required but highly recommended.
        The import date should be formatted as an ISO 8601 date
        (yyyy-mm-ddThh:mm:ssZ) and should
        be the UTC time that the the import was performed. If the date is
        missing, the time the script was started will be used.
    ...
    if split:
        for fprint in fingerprint_list:
            ...
            codes.extend(((c["track_id"].encode("utf-8"),
                           c["fp"].encode("utf-8")) for c in split_prints))
    else:
        docs.extend(fingerprint_list)
        codes.extend(((c["track_id"].encode("utf-8"),
                       c["fp"].encode("utf-8")) for c in fingerprint_list))
...
def query_fp(code_string, rows=15, local=False, get_data=False):
    ...
    try:
        ...
        if get_data:
            ...
            fields = "track_id"
        with solr.pooled_connection(_fp_solr) as host:
            resp = host.query(code_string, qt="/hashq", rows=rows,
                              fields=fields)
        return resp
