...
class ConnectionPool(object):
    ...
    def __init__(self, klass, *args, **kwargs):
        ...
        Provide any addition args or kwargs to pass during initialization of
        new connections.

        If a kwarg named pool_size is provided, it will dictate the maximum
        number of connections to retain in the pool.
...
def reallyunicode(s, encoding="utf-8"):
    ...
    if type(s) is str:
        return s
    if type(s) is bytes:
        for args in ((encoding,), ('utf-8',), ('latin-1',),
                     ('ascii', 'replace')):
            try:
                s = s.decode(*args)
                break
...
def makeNiceLucene(text):
    ...
    # #Escaping%20Special%20Characters
...
class SolrConnection:
    ...
    def __init__(self, url,
        ...
        """
        self.scheme, self.host, self.path = urllib.parse.urlparse(
            url, 'http')[:3]
        self.url = url
    ...
    def smartQuery(self, query, fq='', fields='name,id', sort='', limit=0,
                   start=0, blockSize=1000, callback=None):
        ...
        for startAt in range(start, limit, blockSize):
            ...
            response = self.query(query, fields=fields, rows=blockSize,
                                  start=startAt, sort=sort, fq=fq)
            if len(response) == 0:
                break
    ...
    def query(self, q, fields=None, highlight=None,
        ...
        try:
            ...
            if use_experimental_parser:
                data = self.parse_query_response_python(
                    s3, params=params, connection=self)
            else:
                xml = StringIO(s3)
                data = self.parse_query_response(
                    xml, params=params, connection=self)
    ...
    def _update(self, request, addHandler="/update"):
        ...
        if self.batch_cnt:
            ...
            return
        try:
            rsp = self._post(self.path + addHandler + self.invariant, request,
                             self.xmlheaders)
            data = rsp.read()
        finally:
            if not self.persistent:
        ...
        if data.startswith(b'<result status="') \
                and not data.startswith(b'<result status="0"'):
    ...
    def _reconnect(self):
        ...
        try:
            ...
            print("Error re-connecting. I'm going to wait one minute for solr "
                  "to restart. If it doesn't come back there's a problem.")
            time.sleep(60)
            self.conn.connect()
    ...
    def _cleanup(self, body):
        # clean up the body
        # section 2.2 of the XML spec. Three characters from the 0x00-0x1F
        # block are allowed: 0x09, 0x0A, 0x0D.
...
def stringToPython(f):
    """
    Convert a doc encoded as strings to native python types using EN's
    schema.
    """
    ...
    def utc_from_string(value):
        ...
        try:
            ...
            try:
                ...
                # I've seen a date like this:
                # <date name="d_when">2008-12-03T02:07:52Z</date> ,
                # e.g. no microseconds.
                except ValueError:
